...
``platform_manifest/data/repo_graph.yaml`` and is resolved via
...
def default_config_path() -> Path:
    """Path to the bundled ``data/repo_graph.yaml``.
    ...
    """
    return Path(str(resources.files("platform_manifest") / "data" / "repo_graph.yaml"))
...
def load_default_repo_graph() -> RepoGraph:
    """Load + cache the bundled repo graph. Safe to call from coordinator
